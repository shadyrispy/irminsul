...
use auto_artifactarium::{
    matches_achievement_packet, matches_avatar_packet, matches_item_packet,
...
fn parse_export_settings(json: &str) -> ExportSettings {
    match serde_json::from_str(json) {
        ...
        Err(e) => {
            log_to_android("WARN", &format!("Failed to parse export settings, using defaults: {}", e));
...
pub unsafe extern "system" fn Java_com_esc_irminsul_NativeLib_nativeProcessPacket(
    ...
    for command in &commands {
        if let Some(items) = matches_item_packet(command) {
            log_to_android("INFO", &format!("Matched item packet: {} items", items.len()));
    ...
    let all_complete =
        state.has_items && state.has_avatars && state.has_achievements;

    // Release the lock before calling back into Java
    drop(state_guard);

    if all_complete {
        notify_data_complete(&mut env, artifact_count, weapon_count, material_count, character_count, achievement_count);
    }
...
pub unsafe extern "system" fn Java_com_esc_irminsul_NativeLib_nativeExportGood(
    ...
    let settings = if settings_json.is_null() {
        ...
        let jstr = jni::objects::JString::from(unsafe { jni::objects::JObject::from_raw(settings_json) });
...
pub unsafe extern "system" fn Java_com_esc_irminsul_NativeLib_nativeExportAchievements(
    ...
    let format = match format_code {
        ...
        _ => {
            log_to_android("ERROR", &format!("Unknown achievement format: {}", format_code));
            return std::ptr::null_mut();
        }
    };
